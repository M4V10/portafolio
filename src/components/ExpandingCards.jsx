...
// eslint-disable-next-line no-unused-vars -- 'motion' se usa en JSX como <motion.div>, <motion.h3>, <motion.p>
...
export function ExpandingCards({
  ...
  const { activeWidth, inactiveWidth, titleActive, titleInactive } = useMemo(() => {
  ...
  const handleNext = () => setActiveIndex((prev) => Math.min(cards.length - 1, prev + 1))
  ...
  return (
    <div className="relative group w-full">
      <div
        ref={containerRef}
        className={`flex overflow-x-auto w-full ${gap} ${height} ${classNames?.container || ''}`}
      >
        {cards.map((card, index) => (
          <motion.div
            ...
            className={`relative rounded-2xl overflow-hidden cursor-pointer flex-shrink-0 snap-start ${classNames?.card || ''}`}
            animate={{
          ...
          >
            ...
            />
            <div
              className={`absolute inset-0 bg-gradient-to-b from-transparent to-black/60 ${classNames?.overlay || ''}`}
            />
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6 text-white">
              ...
              <motion.p
                ...
                className={`text-xs md:text-sm ${classNames?.description || ''}`}
              >
                {card.description}
              </motion.p>
            </div>
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        >
          {prevIcon || (
            <ChevronLeft className={`w-5 h-5 md:w-6 md:h-6 text-gray-800 ${classNames?.buttonIcon || ''}`} />
          )}
        </button>
        ...
        <button
          ...
          className={`absolute right-2 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white/50 rounded-full p-2 transition-all pointer-events-auto ${
            activeIndex === cards.length - 1 ? 'opacity-0 cursor-default' : 'opacity-100'
          } ${classNames?.button || ''}`}
        >
          {nextIcon || (
            <ChevronRight className={`w-5 h-5 md:w-6 md:h-6 text-gray-800 ${classNames?.buttonIcon || ''}`} />
          )}
        </button>
      </div>
    </div>
